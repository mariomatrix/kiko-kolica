...
REQUEST_INBOX = Path(settings.REQUEST_INBOX)
RESPONSE_ROOT = Path(settings.RESPONSE_ROOT)
ERROR_ROOT = Path(settings.ERROR_ROOT)
...
def prepare_dataframe(xlsx_path: str, sheet_name: str, tura_name: str):
    logger.info(f"Citam: {xlsx_path} -> sheet: '{sheet_name}'")
